    ...
    <div className="max-w-md">
      ...
      <div className="bg-stone-50 rounded-2xl border border-stone-100 divide-y divide-stone-100 overflow-hidden mb-5">
        ...
        <Row icon={<Crown size={14} className="text-stone-400" />}
          label="Paket" value={`${pkg.emoji} ${pkg.name} Â· ${formatPrice(pkg.price)}`} />
        ...
        {form.akadVenue && (
          <Row icon={<MapPin size={14} className="text-stone-400" />}
            label="Akad" value={`${form.akadVenue}${form.akadDate ? ' Â· ' + new Date(form.akadDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : ''}`} />
        )}
        {form.resepsiVenue && (
          <Row icon={<MapPin size={14} className="text-stone-400" />}
            label="Resepsi" value={`${form.resepsiVenue}${form.resepsiDate ? ' Â· ' + new Date(form.resepsiDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : ''}`} />
        )}
      </div>
