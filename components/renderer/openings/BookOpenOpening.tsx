...
export default function BookOpenOpening({ config, data, meta, onOpen, positionMode = 'fixed', previewGuestName }: Props) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: primary, perspective: '1500px' }}
    >
      {/* Background photo (visible underneath pages) */}
      {bgPhoto && (
        <motion.div
      ...
      <motion.div
        ...
        style={{
          ...
          boxShadow: `inset -12px 0 30px ${primary}44, inset -2px 0 8px ${primary}22`,
          borderRight: `1px solid ${accent}22`,
        }}
        animate={clicked ? { rotateY: -120 } : { rotateY: 0 }}
      ...
      {/* Spine line */}
      <motion.div
        className="absolute top-0 bottom-0 left-1/2 -translate-x-1/2 z-[11]"
        style={{ width: 1, background: `linear-gradient(to bottom, ${accent}11, ${accent}44, ${accent}11)` }}
      ...
      {/* Content - bottom-anchored, above pages */}
      <motion.div
