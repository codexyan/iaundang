...
/* Ornamental vertical bars for gate panels */
const GATE_BARS = [0.15, 0.3, 0.5, 0.7, 0.85]

export default function GateOpenOpening({ config, data, meta, onOpen, positionMode = 'fixed', previewGuestName }: Props) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={clicked ? { rotateY: -90 } : { rotateY: 0 }}
        transition={{ duration: 1, ease: [0.65, 0, 0.35, 1] }}
      >
        {/* Ornamental vertical lines */}
        {GATE_BARS.map((pct, i) => (
          <div key={i} className="absolute top-[10%] bottom-[10%]" style={{
            left: `${pct * 100}%`,
            width: 1,
            background: `linear-gradient(to bottom, transparent, ${accent}30 20%, ${accent}30 80%, transparent)`,
          }} />
        ))}
        {/* Inner edge accent */}
        <div className="absolute top-0 right-0 bottom-0" style={{
          width: 2,
          background: `linear-gradient(to bottom, transparent 5%, ${accent}44 25%, ${accent}44 75%, transparent 95%)`,
        }} />
      </motion.div>
